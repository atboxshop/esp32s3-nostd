...
if [ "${WOKWI_PROJECT_ID}" == "" ]; then
    wokwi-server --chip esp32s3 target/${ESP_ARCH}/${BUILD_MODE}/esp32s3_nostd
else
    wokwi-server --chip esp32s3 --id ${WOKWI_PROJECT_ID} target/${ESP_ARCH}/${BUILD_MODE}/esp32s3_nostd
fi
